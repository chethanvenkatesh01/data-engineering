...
def generate_create_table_ddl(
    warehouse: str, table_name: str, column_definitions: dict
) -> str:
    """
    ...
    if warehouse not in ("BIGQUERY", "SNOWFLAKE"):
        raise ValueError(
            "Unsupported warehouse. Supported values are: 'BIGQUERY', 'SNOWFLAKE'."
        )
    ...
    for column, data_type in column_definitions.items():
        ...
        if not mapped_type:
            raise ValueError(
                f"No mapping found for data type '{data_type}' in warehouse '{warehouse}'."
            )
        escaped_column = escape_column_name(column, warehouse)
...
def generate_insert_ddl(warehouse: str, table_name: str, column_values: dict) -> str:
    ...
    columns = ", ".join(
        escape_column_name(col, warehouse) for col in column_values.keys()
    )
    values = []
