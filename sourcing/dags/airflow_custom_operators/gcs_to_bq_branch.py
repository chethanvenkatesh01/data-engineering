...
from airflow_custom_operators.gcs_to_bq import GCSToBigqueryOperator

class GcsToBQPipeline(BaseOperator, BeamDataflowMixin):
    @apply_defaults
    def __init__(
            self,
            xcom_task_id: str,
            ...
            job_class: Optional[str] = "com.impact.GcsToBigQueryPipeline",
        ...
        self._dataflow_job_name = f"gcs-to-bq-{self.normalized_view_name.replace('_', '-').lower()}-{tenant.lower().replace('_', '-')}-{pipeline}"
    ...
    def execute(self, context):
        ...
        if self.replace_special_chars:
            ...
            self.pipeline_options = {
                ...
                "gcsBlobPathPrefix": self.gcs_blob_path_prefix if self.gcs_blob_path_prefix and self.gcs_blob_path_prefix.strip()!="" and str(self.gcs_blob_path_prefix).lower() not in ["null", "none"] else "",
            ...
            self.pipeline_obj = GCSToBigqueryOperator(
                ...
                partition_column=self.bq_partition_column if self.bq_partition_column!="null" else None,
                clustering_columns=self.bq_clustering_columns.split(",") if self.bq_clustering_columns!="null" else None,
