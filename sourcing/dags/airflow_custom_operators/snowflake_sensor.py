...
class SnowflakeSensor(BaseSensorOperator):
    def __init__(self, *, trigger_mapping_table:str='generic_trigger_mapping', trigger_mapping_df:pd.DataFrame=None, **kwargs) -> None:
        ...
        self.view_trigger_status_mapping:dict = eval(Variable.get("view_trigger_status_mapping", default_var="{}"))
        ...
        if trigger_mapping_df is None:
            ...
            self.trigger_mapping_df = trigger_mapping_df[trigger_mapping_df['view'].isin(self.views)]
        
        self.view_trigger_mandatory_mapping:dict = self.trigger_mapping_df.set_index('view').to_dict()['is_mandatory']
        Variable.set("view_trigger_mandatory_mapping", self.view_trigger_mandatory_mapping)
    ...
    def poke(self, context: Dict):
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            wait(futures)
            log.info("Fetched the trigger status for all views")
        ...
        Variable.set("view_trigger_status_mapping", self.view_trigger_status_mapping)
        ...
        for trigger in mandatory_triggers:
            if trigger in failed_view_trigger_map:
                log.error("Didn't receive all mandatory triggers")
