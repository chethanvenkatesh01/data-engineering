...
class GcsSensor(BaseSensorOperator):
    ...
    def poke(self, context:Dict):
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for idx, future in enumerate(futures):
                ...
                Variable.set(f"{view_list[idx]}_dirs_with_trigger_file", result[1])
                if len(result[0]) == 0:
                    self.view_trigger_status_mapping[view_list[idx]] = True
                else:
                    self.view_trigger_status_mapping[view_list[idx]] = False
                    #failed_view_trigger_map[view_list[idx]] = [row[0] if isinstance(row, tuple) or isinstance(row, list) else row for row in result ]
                    failed_view_trigger_map[view_list[idx]] = result[0]
