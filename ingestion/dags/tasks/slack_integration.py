...
class slack_notification():
    ...
    def send_slack_message(self,message, thread_ts=""):
        ...
        data = {
            ...
            "thread_ts":Variable.get('slack_notification_thread_ts', default_var='') if not thread_ts else thread_ts
        }
        data.update(message)
        Variable.set('delete',self.slack_token)
        ...
        try:
            ...
            Variable.set('delete',response.json())
    ...
    def on_task_failure(self, **context):
        ...
        try:
            ...
            message_list = []
            newline = ' \n '
            for i,t in enumerate(ti.get_dagrun().get_task_instances(state=TaskInstanceState.FAILED)):  # type: TaskInstance
                di = t.dag_id
                tk = t.task_id
                lu = t.log_url
