def add_gcp_connection():
    ...
    for conn_details in connections:
        ...
        if not project_id:
            raise ValueError(f"Project ID for {project_var} is not set in Airflow Variables")
...
def add_psg_connection():
    ...
    curr_conn_id = models.Variable.get('postgres_conn_id', default_var='postgres_default')
