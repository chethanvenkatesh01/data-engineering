...
public class BoundedReadFromUnboundedSource<T> extends PTransform<PBegin, PCollection<T>> {
    ...
    /**
     * Returns a new {@link org.apache.beam.sdk.io.BoundedReadFromUnboundedSource} that reads a bounded amount of data from
     ...
     */
    public com.impact.utils.io.BoundedReadFromUnboundedSource<T> withMaxNumRecords(long maxNumRecords) {
        return new com.impact.utils.io.BoundedReadFromUnboundedSource<>(source, maxNumRecords, maxReadTime);
    }
     ...
     */
    public com.impact.utils.io.BoundedReadFromUnboundedSource<T> withMaxReadTime(Duration maxReadTime) {
        return new com.impact.utils.io.BoundedReadFromUnboundedSource<>(source, maxNumRecords, maxReadTime);
    }
    ...
    public PCollection<T> expand(PBegin input) {
        Coder<com.impact.utils.io.BoundedReadFromUnboundedSource.Shard<T>> shardCoder = SerializableCoder.of((Class<com.impact.utils.io.BoundedReadFromUnboundedSource.Shard<T>>) (Class) com.impact.utils.io.BoundedReadFromUnboundedSource.Shard.class);
        PCollection<ValueWithRecordId<T>> read =
                input
                        ...
                        .apply("Split", ParDo.of(new com.impact.utils.io.BoundedReadFromUnboundedSource.SplitFn<>()))
                        .setCoder(shardCoder)
                        .apply("Reshuffle", Reshuffle.viaRandomKey())
                        .apply("Read", ParDo.of(new com.impact.utils.io.BoundedReadFromUnboundedSource.ReadFn<>()))
    ...
    private static class SplitFn<T> extends DoFn<com.impact.utils.io.BoundedReadFromUnboundedSource.Shard<T>, com.impact.utils.io.BoundedReadFromUnboundedSource.Shard<T>> {
        /**
        ...
        @ProcessElement
        public void process(
                @Element com.impact.utils.io.BoundedReadFromUnboundedSource.Shard<T> shard, OutputReceiver<com.impact.utils.io.BoundedReadFromUnboundedSource.Shard<T>> out, PipelineOptions options)
                throws Exception {
    ...
    private static class ReadFn<T> extends DoFn<com.impact.utils.io.BoundedReadFromUnboundedSource.Shard<T>, ValueWithRecordId<T>> {
        @ProcessElement
//        @SuppressFBWarnings(
//                value = "RCN_REDUNDANT_NULLCHECK_WOULD_HAVE_BEEN_A_NPE",
//                justification = "https://github.com/spotbugs/spotbugs/issues/756")
        public void process(
                @Element com.impact.utils.io.BoundedReadFromUnboundedSource.Shard<T> shard, OutputReceiver<ValueWithRecordId<T>> out, PipelineOptions options)
                throws Exception {
            Instant endTime =
    ...
    abstract static class Shard<T> implements Serializable {
        ...
        abstract com.impact.utils.io.BoundedReadFromUnboundedSource.Shard.Builder<T> toBuilder();

        @AutoValue.Builder
        abstract static class Builder<T> {
            abstract com.impact.utils.io.BoundedReadFromUnboundedSource.Shard.Builder<T> setSource(UnboundedSource<T, ?> source);

            abstract com.impact.utils.io.BoundedReadFromUnboundedSource.Shard.Builder<T> setMaxNumRecords(long maxNumRecords);

            abstract com.impact.utils.io.BoundedReadFromUnboundedSource.Shard.Builder<T> setMaxReadTime(@Nullable Duration maxReadTime);

            abstract com.impact.utils.io.BoundedReadFromUnboundedSource.Shard<T> build();
        }
    }
}
