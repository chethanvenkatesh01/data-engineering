 ...
 * org.apache.beam.sdk.io.FileBasedSource} that is able to read the decompressed file format.
 *
 ...
 * compression types are supported by implementing a {@link com.impact.utils.io.CompressedSource.DecompressingChannelFactory}.
 *
...
public class CompressedSource<T> extends FileBasedSource<T> {
    ...
    @Deprecated
    public enum CompressionMode implements com.impact.utils.io.CompressedSource.DecompressingChannelFactory {
        ...
        static com.impact.utils.io.CompressedSource.DecompressingChannelFactory fromCanonical(Compression compression) {
            switch (compression) {
                case AUTO:
                    return AUTO;
    ...
    private final com.impact.utils.io.CompressedSource.DecompressingChannelFactory channelFactory;
     ...
     * {@link com.impact.utils.io.CompressedSource#withDecompression}.
     */
    public static <T> com.impact.utils.io.CompressedSource<T> from(FileBasedSource<T> sourceDelegate) {
        return new com.impact.utils.io.CompressedSource<>(sourceDelegate, com.impact.utils.io.CompressedSource.CompressionMode.AUTO);
    }

    /**
     * Return a {@code CompressedSource} that is like this one but will decompress its underlying file
     * with the given {@link com.impact.utils.io.CompressedSource.DecompressingChannelFactory}.
     */
    public com.impact.utils.io.CompressedSource<T> withDecompression(com.impact.utils.io.CompressedSource.DecompressingChannelFactory channelFactory) {
        return new com.impact.utils.io.CompressedSource<>(this.sourceDelegate, channelFactory);
    }

    /** Like {@link #withDecompression} but takes a canonical {@link Compression}. */
    public com.impact.utils.io.CompressedSource<T> withCompression(Compression compression) {
        return withDecompression(com.impact.utils.io.CompressedSource.CompressionMode.fromCanonical(compression));
    }
    ...
    private CompressedSource(
            FileBasedSource<T> sourceDelegate, com.impact.utils.io.CompressedSource.DecompressingChannelFactory channelFactory) {
        super(
     ...
     * com.impact.utils.io.CompressedSource#createForSubrangeOfFile}.
     */
    private CompressedSource(
            FileBasedSource<T> sourceDelegate,
            com.impact.utils.io.CompressedSource.DecompressingChannelFactory channelFactory,
            Metadata metadata,
            long minBundleSize,
            long startOffset,
            long endOffset) {
    ...
    protected FileBasedSource<T> createForSubrangeOfFile(Metadata metadata, long start, long end) {
        return new com.impact.utils.io.CompressedSource<>(
    ...
    protected final boolean isSplittable() {
        ...
        if (channelFactory == com.impact.utils.io.CompressedSource.CompressionMode.UNCOMPRESSED) {
            return true;
        }
        if (channelFactory == com.impact.utils.io.CompressedSource.CompressionMode.AUTO) {
    ...
    protected final FileBasedReader<T> createSingleFileReader(PipelineOptions options) {
        ...
        return new com.impact.utils.io.CompressedSource.CompressedReader<>(this, sourceDelegate.createSingleFileReader(options));
    }
    ...
    public final com.impact.utils.io.CompressedSource.DecompressingChannelFactory getChannelFactory() {
        return channelFactory;
    }

    /**
     * Reader for a {@link com.impact.utils.io.CompressedSource}. Decompresses its input and uses a delegate reader to
    ...
    public static class CompressedReader<T> extends FileBasedReader<T> {
        ...
        @GuardedBy("progressLock")
        private com.impact.utils.io.CompressedSource.CompressedReader.CountingChannel channel;
        //private @Nullable com.impact.utils.io.CompressedSource.CompressedReader.CountingChannel channel;

        private com.impact.utils.io.CompressedSource.DecompressingChannelFactory channelFactory;

        /** Create a {@code CompressedReader} from a {@code CompressedSource} and delegate reader. */
        public CompressedReader(com.impact.utils.io.CompressedSource<T> source, FileBasedReader<T> readerDelegate) {
            super(source);
        ...
        protected final void startReading(ReadableByteChannel channel) throws IOException {
            synchronized (progressLock) {
                this.channel = new com.impact.utils.io.CompressedSource.CompressedReader.CountingChannel(channel, getCurrentSource().getStartOffset());
