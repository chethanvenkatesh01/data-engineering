...
public class DbToSfPipeline {
    ...
    public static void main(String [] args) throws Exception {
        DbToSfPipelineOptions options = PipelineOptionsFactory.fromArgs(args).withValidation().as(DbToSfPipelineOptions.class);
        run(options);
    }

    public static SnowflakeIO.UserDataMapper<TableRow> getCsvMapper() {
        return (SnowflakeIO.UserDataMapper<TableRow> ) recordLine -> {
            ...
            for (String key : recordLine.keySet()) {
                Object value = recordLine.get(key);
                // Convert the value to a string (handling nulls)
                stringList.add(value != null ? value.toString() : null);
            }

            // Convert the ArrayList to an array of Strings and return
            return stringList.toArray(new String[0]);
        };
    }

    private static void run(DbToSfPipelineOptions options) throws Exception {
        ...
        String snowflakeClusteringColumns = options.getSnowflakeClusteringColumns();
        boolean replaceTable = options.getReplaceTable();
...
//                ? null : options.getDbIncrementalColumnValue();
        ...
        if(queries.length > 0 && !queries[0].equals("")) {
            ...
            Map<String, String> dbSchema = dbManager.getTableSchema(firstQuery, dbTable, schemaName, dbType, snowflakeTable);
            Map<String, String> sfSchema = SnowflakeSchemaConversion.dbToSf(dbSchema, dbType);
            ...
            for(int i=0; i<queries.length; i++) {
                p.apply(String.format("Read from DB part-%s", i+1), JdbcIO.<TableRow>read().withDataSourceConfiguration(
                                ...
                                .withQuery(queries[i])
                                .withRowMapper(new MapToTableRow(dbSchema, sfSchema, dbType, replaceSpecialChars)))
                        .apply("Write to Snowflake", SnowflakeIO.<TableRow>write()
