...
error=$(sudo gcloud artifacts repositories create de-$module --repository-format=docker --location=us-central1 || echo "failed")
if [ "$error" = "failed" ]; then
  echo "Repository already exists de-$module. Skipping..."
fi
...
# install keda
sudo helm repo add kedacore https://kedacore.github.io/charts
